...
export default function Navbar({ 
  ...
  const navItems = [
    ...
    { id: 'match', label: 'Smart Match', icon: Sparkles, highlight: true },
  ];
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
              ...
              return (
                ...
                >
                  ...
                  {item.badge !== undefined && item.badge !== null && (
                    <span className="ml-0.5 px-1.5 py-0.2 text-xs font-bold rounded-full bg-rose-100 text-rose-600">
                      {item.badge}
                    </span>
                  )}
                  {item.highlight && (
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-sky-700 bg-sky-50 border border-sky-200 rounded-full hover:bg-sky-100 transition shadow-sm"
              title="Ask Club Assistant (Phase 25)"
            >
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className="w-full px-4 py-2 text-left text-sm text-sky-700 hover:bg-sky-50 flex items-center gap-2.5 font-medium"
                      >
        ...
        {/* Mobile Nav bar bottom strip */}
        <div className="flex md:hidden border-t border-slate-200 py-2 gap-1 overflow-x-auto no-scrollbar">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            return (
              ...
              >
                ...
                {item.label}
                {item.badge && (
                  <span className="px-1 text-[10px] font-bold rounded-full bg-rose-100 text-rose-600">
                    {item.badge}
                  </span>
                )}
              </button>
